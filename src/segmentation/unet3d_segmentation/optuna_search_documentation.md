# Optuna Hyperparameter Search - Documentación

## Descripción General

`optuna_search.py` realiza una **búsqueda automatizada y optimizada de hiperparámetros** para el modelo U-Net 3D usando Optuna. El objetivo es encontrar la mejor combinación de parámetros que maximice la métrica `val_dice` (validación Dice coefficient).

## Funcionalidad Principal

### 1. **Búsqueda Automática de Hiperparámetros (20 trials)**

Optuna ejecuta **20 entrenamientos independientes** con diferentes combinaciones de parámetros, cada uno completo (100 epochs).

**Parámetros explorados:**

| Parámetro | Rango/Opciones | Descripción |
|-----------|---|---|
| **lr** (learning rate) | [1e-4, 1e-3] continuo | Tasa de aprendizaje del optimizador Adam (búsqueda logarítmica) |
| **base_channels** | [16, 24, 32, 48] discreto | Canales base en primera capa del U-Net (afecta capacidad modelo) |
| **bce_weight** | [0.2, 0.8] continuo | Peso de la pérdida BCE en la loss total (0.0-1.0) |
| **dice_weight** | 1.0 - bce_weight | Peso automático de la pérdida Dice (complemento BCE) |
| **weight_decay** | [1e-6, 1e-4] continuo | Regularización L2 del optimizador (evita overfitting) |
| **lr_patience** | [3, 5, 10] discreto | Paciencia del ReduceLROnPlateau scheduler (epochs sin mejora antes de reducir lr) |

### 2. **Batch Size Adaptativo**

El batch size se ajusta automáticamente según `base_channels` para optimizar VRAM:

```
base_channels ≤ 16  → batch_size = 4
base_channels ≤ 32  → batch_size = 3
base_channels > 32  → batch_size = 2
```

### 3. **Ejecución de Entrenamientos**

Para cada trial, se ejecuta el comando:

```bash
python src/segmentation/unet3d_segmentation/train_unet3d_baseline.py \
    --lr <valor> \
    --base_channels <valor> \
    --batch_size <valor_adaptativo> \
  --save_dir output/optuna_3d/trial_{id}_lr_.../ \
    --epochs 100 \
    --bce_weight <valor> \
    --dice_weight <valor> \
    --weight_decay <valor> \
    --lr_patience <valor> \
    --patience 30
```

**Cada entrenamiento:**
- Entrena por **100 epochs** (o menos si early stopping a paciencia 30)
- Guarda checkpoints en `output/optuna_3d/trial_{id}_*/`
- Genera `summary.json` con métricas finales

### 4. **Recolección de Resultados**

Después de cada entrenamiento:
1. Lee el archivo `summary.json` de ese trial
2. Extrae el valor `best_val_dice` (mejor Dice en validación)
3. Retorna ese valor a Optuna

### 5. **Base de Datos Persistente (SQLite)**

Usa `output/optuna_3d/optuna_study.db` para almacenar:
- Todos los trials ejecutados
- Parámetros de cada trial
- Resultados (val_dice)
- Mejor trial encontrado

**Ventaja:** Si el script se interrumpe, puede reanudarse sin perder datos.

### 6. **Información en Consola**

Para cada trial imprime:

```
[Trial 0] Running with:
  lr=5.3e-04, channels=32, batch=3
  bce_weight=0.45, dice_weight=0.55
  weight_decay=2.5e-05, lr_patience=5
[Trial 0] Best val Dice: 0.7523
```

### 7. **Resultado Final**

Al terminar todos los 20 trials, imprime:

```
============================================================
BEST RESULT:
============================================================
Best val Dice: 0.8234
Best params: {'lr': 0.0003, 'base_channels': 32, ...}
Best trial: 7
============================================================
```

## Comparación: Antes vs Después

| Aspecto | Antes (4 trials) | Ahora (20 trials) |
|--------|-----------------|-------------------|
| **Combinaciones** | 2 × 2 = 4 | ~5^6 = miles de combinaciones |
| **Learning rates** | [1e-3, 3e-4] | [1e-4, 1e-3] continuo |
| **Base channels** | [16, 32] | [16, 24, 32, 48] |
| **Balance loss** | 50/50 fijo | Variable [0.2, 0.8] |
| **Regularización** | No | weight_decay variable |
| **Scheduler** | No configurable | Configurable lr_patience |
| **Epochs** | 80 | 100 |
| **Almacenamiento** | En memoria | SQLite persistente |

## Tiempo Estimado

- **1 trial (~100 epochs):** ~15-30 min (depende GPU)
- **20 trials:** ~5-10 horas total
- Puede dejarse ejecutando en background

## Cómo Usar

```bash
python src/segmentation/unet3d_segmentation/optuna_search.py
```

### Reanudar búsqueda (si se corta)

```bash
python src/segmentation/unet3d_segmentation/optuna_search.py
```

Optuna detectará automáticamente `output/optuna_3d/optuna_study.db` y continuará desde donde se quedó.

### Ver resultados intermedios

La base de datos SQLite puede consultarse:

```bash
sqlite3 output/optuna_3d/optuna_study.db
SELECT trial_id, value FROM trials ORDER BY value DESC LIMIT 5;
```

## Objetivo

Mejorar el baseline de **0.8007 Dice (80.07%)** encontrado anteriormente con:
- lr=0.0003
- base_channels=32

Esperanza: encontrar combinaciones que alcancen **82-85% Dice** o superior.
