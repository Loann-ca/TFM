...
Usage:
    python train_unet2d_baseline.py --output_dir output --epochs 50 --batch_size 16
"""
...
def maybe_augment_2d(ct: np.ndarray, mask: np.ndarray, rng: np.random.RandomState) -> Tuple[np.ndarray, np.ndarray]:
    """Apply simple geometry-preserving augmentation to a CT/mask slice pair."""
    if rng.rand() < 0.5:
        ct = np.flip(ct, axis=0)
        mask = np.flip(mask, axis=0)
    if rng.rand() < 0.5:
...
class FullVolumeNodule2DDataset(Dataset):
    ...
    For each patient, it generates ``patches_per_patient`` samples:
      - Around half centered on nodule coordinates from metadata (positive mining).
      - The rest from random spatial positions/slices (background diversity).

    All patches have fixed size ``patch_size x patch_size``.
    """
    ...
    def __init__(
        ...
        meta = pd.read_csv(meta_path)
        self.items: List[SlicePatchItem] = []
        half = patch_size // 2
        rng = np.random.RandomState(seed)
        ...
        for patient_id, group in meta.groupby("patient_id"):
            ...
            nodule_points: List[Tuple[int, int, int]] = []
            for _, row in group.iterrows():
                cx, cy, cz = _bbox_center(str(row.bbox_x), str(row.bbox_y), str(row.bbox_z))
                cx = int(np.clip(cx, half, max_x))
                cy = int(np.clip(cy, half, max_y))
                cz = int(np.clip(cz, 0, max_z))
                nodule_points.append((cx, cy, cz))
            ...
            for i in range(n_nodule):
                cx, cy, cz = nodule_points[i % len(nodule_points)]
                self.items.append(
                    SlicePatchItem(
                        ct_path=ct_path,
                        mask_path=mask_path,
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        ct = np.array(
            ct_full[cx - half:cx + half, cy - half:cy + half, z],
            dtype=np.float32,
        )
        mask = np.array(
            mask_full[cx - half:cx + half, cy - half:cy + half, z],
            dtype=np.float32,
        )
        mask = (mask > 0.5).astype(np.float32)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--patch_size", type=int, default=128, help="Patch side length in pixels (default: 128)")
    parser.add_argument("--patches_per_patient", type=int, default=32, help="2D patches generated per patient per epoch (default: 32)")
    parser.add_argument("--base_channels", type=int, default=32, help="U-Net base channels")
    parser.add_argument("--bce_weight", type=float, default=0.5, help="Weight for BCE loss")
    parser.add_argument("--dice_weight", type=float, default=0.5, help="Weight for Dice loss")
    parser.add_argument("--patience", type=int, default=25, help="Early stopping patience (epochs without improvement)")
